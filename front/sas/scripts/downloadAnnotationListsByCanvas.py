...
def fetch(url, retry=0):
    ...
    if os.path.exists(cached):
        ...
        try:
            ...
            logging.error(
                "Getting %s failed due to %s: %s (Rery: %s)",
                url,
                error.code,
                error.reason,
                retry,
            ...
            # 404 means no annotations for this canvas
            logging.error(
                "Failed to get %s due to %s. Do you have the correct URL for SAS and is it running?",
                url,
                error,
            )
...
if __name__ == "__main__":
    ...
    for canvas in manifest["sequences"][0]["canvases"]:
        count += 1
        annoListData = fetch(
            "%s/annotation/search?uri=%s" % (sasEndpoint, canvas["@id"])
        )
        if annoListData:

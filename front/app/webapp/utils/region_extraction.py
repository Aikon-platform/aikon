...
def create_empty_regions(digit: Digitization):
    ...
    try:
        ...
        log(
            f"[create_empty_regions] Unable to create new RegionExtraction for digit #{digit.id} in the database",
            e,
        )
        return False
    ...
    try:
        ...
        if not success:
            log(
                f"[create_empty_regions] unable to index manifest in SAS for RegionExtraction #{regions.id}."
        ...
        log(
            f"[create_empty_regions] Error when indexing manifest in SAS for RegionExtraction #{regions.id}."
            f"Deleting RegionExtraction record.",
            e,
        )
        regions.delete()

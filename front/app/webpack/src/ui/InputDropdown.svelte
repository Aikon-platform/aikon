<!-- a svelte implementation of Choices.js.
    ...
    - select all / unselect all choices is possible from the dropdown
    - reset the dropdown to its start state (see in good to know)
    - buffered emitting of newly set values, to limit events to what's strictly necessary
    - prefixes, svg icons or iconify icons can be set for each option
    - specific styles
    - provides a lightDisplay option, for a more discreet view

    emits:
    - "updateValues" @type {array}
    ...
    - it does not handle updates to props

    good to know:
    - about resetting: the ancestor must define a context with `setContext("resetTrigger")` holding a `writable` store. when the store's value changes, resetting is triggered. see `InputSlider.svelte` for more details.
...
<script>
    ...
    /** @type {writable} the `resetTrigger` context stores a writable. we subscribe to the writable, and when its value is updated, reset the dropdown. see the documentation of `InputSlider.svelte` for more info */

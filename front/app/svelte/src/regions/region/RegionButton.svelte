<script>
    ...
    const title = appLang === "fr" ? "Éditer les métadonnées" : "Edit metadata";

    const witness = getContext('witness');
    const wlo = `${window.location.origin}/${appName}`
    const url = `${wlo}/witness/${witness.id}/region/${currentRegion.ref}`;
</script>

<a href={url} class="button tag" title={title}>
